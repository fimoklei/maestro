...
- Never put `apm` prose in an HTTP response — a failure is stated from the server's own message table, never from what apm printed. A named field derived from apm's output may cross only with a shape check where that output is first read, and a cell failing it fails the whole read (ADR-0018; `parseOutdated` is the one precedent).
