...
- **Local server (enforced).** `server.ts` binds `127.0.0.1` only, never `0.0.0.0`. Write routes (`POST`) pass through the `originHostGuard` (`packages/server/src/origin-host-guard.ts`): JSON body + allowlisted `Host` + a present, allowlisted `Origin`. It is a `createApp` dep — production always enables it, tests construct it disabled; no static bypass header. → blocks DNS-rebinding / CSRF: a malicious site POSTing to `localhost:<port>` to drive your machine.
