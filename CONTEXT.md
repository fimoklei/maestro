...
| The sidebar's Harness button | **Harness menu** | The accessible name of the button naming the connected Harness, its release and its skill count. It opens the menu that holds **Harness location** (#991). |
...
| Files outside the selection | **Extra files deployed** | The line in a target's detail pane when the deployment record holds files that belong to no selected skill; a fact to know, not an action to take (ADR-0031). |
...
| Browse dialog title | a noun phrase | Name what the dialog picks — **Inventory folder**, **Folder to clone into**, **Repositories to register** — never an imperative. |
