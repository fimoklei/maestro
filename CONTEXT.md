...
| An operation that never finished | **Unfinished operation** | The durable record of what a Deploy or Remove set out to do — target, Harness, operation, chosen release, previous and desired **Selection**, affected tools — written before the first mutation and cleared only once disk, the manifest and the deployment record agree with it. A matching tag alone never clears it, and it survives a restart. Its notices are **Deploy incomplete** with **Retry deploy** and **Removal incomplete** with **Retry removal**; a retry re-runs the saved release and Selection, and asks for local-copy consent afresh (ADR-0031, #951). |
