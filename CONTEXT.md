...
| Drift | **Behind** | A target's reading, stated on its **Release head**. On a skill's row it means one of the skill's targets is behind and the skill changed in the newer release; a skill has no release of its own. Use *an update is available* as explanation, not as a second status name. The `?` marker reads **Update check did not run**; *drift* never reaches the screen. **Behind** claims a newer release exists and nothing more (ADR-0027); the changed count beside it carries the content fact. *Older tag* is retired (ADR-0031). |
...
| View options | **Filter** / **Display** | The two icon-only controls in the Inventory's band 2 beside **Re-read Inventory**. **Filter** narrows by type and status and carries the count of active filters; **Display** groups the rows by type or status, worst status first, and switches columns off and on. |
