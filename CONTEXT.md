...
| Half-landed Update | **Mixed releases** | A target card whose files sit partly on the chosen release after an Update stopped; its notice is **Update incomplete** and its control **Retry update**, which runs the same chosen release again. Never a target that still holds per-skill dependencies (#933 names that). |
