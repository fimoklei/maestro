...
| Drift | **Behind** | A target's reading, stated in its **Status** column. On a skill's row it means one of the skill's targets is behind and the skill changed in the newer release; a skill has no release of its own. Use *an update is available* as explanation, not as a second status name. The `?` marker reads **Update check did not run**; *drift* never reaches the screen. **Behind** claims a newer release exists and nothing more (ADR-0027); the changed count beside it carries the content fact. *Older tag* is retired (ADR-0031). |
| Changed / unchanged skill | **Changed** / **Unchanged** | The two sections of the **Update target** preview, and the count in a target's Status hover card (`2 of 5 skills changed`). Explain *unchanged* as *identical at both releases*; never *same content*, which is **Content drift**'s territory. |
...
| Promote | **Propose change** / **proposed change** | *Propose change* is the action, the first item of its row's menu and the first button in the row's detail pane; the noun is a **proposed change**. Pushing alone does not mean a pull request exists or review has started. |
...
| Harness row | **Type** / **Name** / **Status** / **Pull request** / **Also in** | The columns of the **Harness** table, one group per stage; band 2 states **Origin**, **Released** and **Branch**. The pull request number links to GitHub, and its hover card reads the request's state as **Open**, **Draft**, **Merged** or **Closed** (#994). |
...
| Freshness reading | **Read** | The one word for reading, on every screen: *Read 4 min ago*, *Read from GitHub 4 min ago*, *Not read yet*, *Read failed — never read*. Only the Harness and Deploy-state show it, because only their data ages; it dates the oldest reading on the screen. While a Harness read runs, its status region says *Reading GitHub…*; nothing on screen does. *Fetch* and *fetched* are retired from user-facing copy; they survive only as the wire field `lastFetchedAt` and the `fetched` / `fetch-failed` outcome values. |
...
| Register a consuming repo | **Register repository** | The one control that registers a repository, in band 1 of **Repositories**. It opens *Register a repository*: one **Folder path** field, one folder per registration, every refusal under the field (#991, #1009). |
| The sidebar's Harness button | **Harness menu** | The accessible name of the button naming the connected Harness, its release and its skill count. It opens the menu that holds **Settings** (#991, #995). |
| Withdraw a registration | **Unregister** | Maestro stops tracking a registered repository; the folder and everything deployed in it stay as they are. Never *Remove* (a deployed primitive) or *Delete* (a Harness skill). The row's menu item is **Unregister**, the dialog reads *Unregister {name}* and confirms with **Unregister repository** (#1009). |
| Repositories row reading | **Ready** / **Folder missing** / **Not a Git repository** | The **Status** column on **Repositories**, beside **Repository** and **Folder path**, read from the folder alone: *Ready* means the folder is there and is a Git repository. A release, a status and a skill count belong to **Deploy-state** (#1009). |
...
| Deploy-state row | **Target** / **Release** / **Status** / **Skills** | The four columns of **Deploy-state**, one row per target under the group headers **Global** and **Repositories**. **Release** holds `vX`, or `vX → vY` when a newer release exists; the Status hover card adds `Newer release vY: N of M skills changed` and *Compared with the Harness, read …*. When the changed count cannot be read, that line reads *Changes could not be read*. The retired *Release head* stated the same on a card (#932, #993). |
| Adopt a release | **Update target** | The one control and dialog that moves a target's whole selection to the newest release; never *Update skill* on a row, which retires (#932). The preview's status badge for a release that changes no selected skill is **No content changes**. |
| Half-landed Update | **Mixed releases** | A target whose files sit partly on the chosen release after an Update stopped; its notice is **Update incomplete** and its control **Retry update**, which runs the **Pending release** again. Never a target that still holds per-skill dependencies, which reads **Pinned per skill**. |
| Per-skill dependencies | **Pinned per skill** | The status of a target whose skills were deployed one at a time, before it followed one release: a neutral Status with the line `3 skills at v0.3.1` and **Release not adopted** in its detail pane. It has no **Update target**; the way to one release is **Remove skill** for each skill, then **Deploy skill**. Deploy refuses such a target with `Left alone — pinned per skill` (#933). |
| Add to a selection | **Deploy skill** | From the Inventory, adds a skill to a target's selection at the target's release; an empty target pins the latest release. From a skill's detail pane it opens the bulk deploy dialog with that skill alone. **Update target** on a behind target's row in that pane prices the move with the skill — a second entrance to the same Update, never a second flow (#937, #1065). Bulk deploy never moves a release. |
...
| Files outside the selection | **Extra files** | The fact in a target's detail pane when the deployment record holds files that belong to no selected skill; a fact to know, not an action to take (ADR-0031). |
...
| Inventory row reading | **Up to date** / **Behind** / **Unknown** / **Not deployed** | The **Status** column on **Inventory**: one badge per skill, the worst reading across its targets. *Unknown* is a check or a read that could not answer; nothing shows until every read has answered. **Targets** beside it counts the targets the skill is deployed to, `—` at zero (#992). |
| View options | **Filter** / **Display** | The two icon-only controls in band 2 beside **Re-read Inventory** and **Re-read Deploy-state**. **Filter** narrows by type (Deploy-state: kind) and status and carries the count of active filters; **Display** groups the rows by type or status, worst status first, and switches columns off and on. |
| Target roll-up | **In sync** / **Local edits** | A whole target's Status on **Deploy-state**. A target where any deployed skill has **Local edits** reads **Local edits**, below **Mixed releases** and **Attention** and above **Behind** and **Pinned per skill**; an **Unverified** copy leaves the target's reading alone (#1131). A single skill is **Up to date**, never *in sync*; a skill row in the target's pane carries one mark, first match wins: **Local edits**, **Unverified**, **No longer released**, **Behind**, **Up to date** (#993). |
| System folder picker | **Folder chooser** | The operating system's own window that **Browse** opens beside a folder field; never *picker* or *dialog* on screen. Where the computer has none, **Browse** is absent and the field takes a typed path (ADR-0032). |
