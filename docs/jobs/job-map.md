...
| ↳ See local divergence from central | Future | When I have edited a deployed primitive locally, I want to see that it no longer matches central, so that I know which copies are modified before I reset or update them. Also refines the inventory-side "already synced" affordance (#112): a content-drifted copy reads "synced, locally modified", not a plain "synced". | A |
...
| ↳ Backfill a newly-detected global tool | Future | When I install a second tool (e.g. Codex) after already deploying globally to Claude Code, I want the cockpit to offer to bring the new tool up to the same set, so that my tools stay in sync instead of the new one starting empty. Global targets are detected live (per ADR-0011); MVP1 only shows the gap honestly (the new tool's card reads "nothing deployed here"), this job closes it. | B, A |
