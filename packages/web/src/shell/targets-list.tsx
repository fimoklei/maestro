...
// One row per detected tool plus one per registered repo. Each `▲N` joins
// drift against deployed primitives, so orphan-behind never inflates N, and
// an unverifiable check reads "unknown", never in sync (J04).
export function TargetsList() {
  ...
  // Whole registered set drives each label, so a tail stays distinct (#211).
  const repoPaths = repos.map((repo) => repo.path);

  // A failed refetch keeps stale tool data — each row must know the read
  // errored and drop to "unknown" (J04), never read a cached name as in sync.
  const toolsRead = {
    data: globalDeploy.data,
    isError: globalDeploy.isError,
  };

  // A bare empty list would read as "no global targets" — one placeholder row
